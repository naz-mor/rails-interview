...
describe TodoLists::CompleteAllController do
  ...
  describe 'POST create' do
    ...
    it 'assigns the reloaded paginated todo items' do
      10.times { |index| todo_list.todo_items.create!(name: "Extra #{index}") }

      post :create, params: { todo_list_id: todo_list.id }

      expect(assigns(:todo_items)).to eq(todo_list.todo_items.limit(10).to_a)
      expect(assigns(:todo_items).all?(&:completed?)).to be(true)
    end
